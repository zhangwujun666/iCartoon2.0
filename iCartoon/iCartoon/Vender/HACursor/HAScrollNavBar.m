...
#define ItemWidth                 TRANS_VALUE(80.0f)
...
//            }
            button.titleLabel.font = [UIFont systemFontOfSize:TRANS_VALUE(13.0f)];
            ((UIView *)[button viewWithTag:333]).hidden = NO;
            _currectItem = button;
        }
    }
}
...
- (void)layoutButtons{
    ...
    for (NSInteger i = 0; i < itemsCount; i++) {
        if (i != itemsCount) {
            ...
            self.itemW = buttonW;
            ((UIView *)[button viewWithTag:333]).frame = CGRectMake(5, buttonH - 2.5f, buttonW - CONVER_VALUE(2.5f), CONVER_VALUE(2.5f));
        }
    }
...
- (UIButton *)createItemWithTitle:(NSString *)title{
    ...
    UIView *divider = [[UIView alloc] initWithFrame:CGRectMake(0, button.frame.size.height - 3.5f, button.frame.size.width, 3.5f)];
    //TODO -- 设置下划线
    divider.backgroundColor = I_COLOR_YELLOW;
...
- (void)updatePageView:(NSNotification *)notifition{
    ...
    for (NSString *key in self.tmpKeys) {
        NSLog(@"key ---> %@  count ---> %ld",key,(unsigned long)self.tmpKeys.count);
...
- (void)clickButtonWhenNotGraduallyChangFont:(UIButton *)button{
    ...
    if (self.minFontSize) {
        self.minFontSize = TRANS_VALUE(13.0f);
...
//        _oldItem.titleLabel.font = [UIFont systemFontOfSize:FontMinSize];
        _oldItem.titleLabel.font = [UIFont systemFontOfSize:TRANS_VALUE(13.0f)];
    }
    ...
    if (self.maxFontSize) {
        self.maxFontSize = TRANS_VALUE(13.0f);
        _currectItem.titleLabel.font = [UIFont systemFontOfSize:self.maxFontSize];
    }else{
//        _currectItem.titleLabel.font = [UIFont boldSystemFontOfSize:FontDetLeSize + FontDefSize];
        _currectItem.titleLabel.font = [UIFont systemFontOfSize:TRANS_VALUE(13.0f)];
    }
}
...
- (void)buttonClick:(UIButton *)button{
    ...
    NSInteger num = offX / [UIScreen mainScreen].bounds.size.width;
    if([self.viewDelegate respondsToSelector:@selector(navibar:didSelectAtIndex:)]) {
        [self.viewDelegate navibar:self didSelectAtIndex:num];
    }
}
...
- (void)buttonMoveAnimationWithIndex:(NSInteger)index {
    for(NSString *key in self.tmpKeys) {
        UIButton *button = [self.itemsDic objectForKey:key];
        [(UIView *)button viewWithTag:333].hidden = YES;
    }
    ...
    if(selectButton.selected) {
        [(UIView *)selectButton viewWithTag:333].hidden = NO;
    } else {
        [(UIView *)selectButton viewWithTag:333].hidden = YES;
    }
}
...
- (void)setItemFontSizeWithFrontItem:(UIButton *)frontItem AndBackItem:(UIButton *)backItem andPrecent:(CGFloat)p{
    ...
    if (self.isGraduallyChangFont) {
        ...
        fontSize1 = TRANS_VALUE(13.0f);
        fontSize2 = TRANS_VALUE(13.0f);
...
- (void)setupNormalFontSizeItem{
    if (self.minFontSize) {
        ...
        self.firstButton.titleLabel.font = [UIFont systemFontOfSize:TRANS_VALUE(13.0f)];
        self.secButton.titleLabel.font = [UIFont systemFontOfSize:TRANS_VALUE(13.0f)];
    }
