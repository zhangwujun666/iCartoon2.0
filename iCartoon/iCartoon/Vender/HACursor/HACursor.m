...
@interface HACursor()<UIScrollViewDelegate, HAScrollNavBarDelegate>
...
- (HAScrollNavBar *)scrollNavBar{
    if (!_scrollNavBar) {
        ...
        _scrollNavBar.backgroundColor = [UIColor whiteColor];
    }
    _scrollNavBar.viewDelegate = self;
...
- (void)setSelectIndex:(NSInteger)selectIndex {
    self.scrollNavBar.selectIndex = selectIndex;
    if([self.delegate respondsToSelector:@selector(cursor:selectedAtIndex:)]) {
        [self.delegate cursor:self selectedAtIndex:selectIndex];
    }
...
- (void)layoutWhenHiddenSortbutton{
    //不显示排序按钮的布局
    CGFloat scrollX         = -6.0f;
    CGFloat scrollY         = 0;
    CGFloat scrollH         = TRANS_VALUE(34.0f);
    self.navBarH            = scrollH;
    CGFloat scrollW         = 12.0f+self.width;
    self.scrollNavBar.frame = CGRectMake(scrollX, scrollY, scrollW, scrollH);
    self.dividerView.frame = CGRectMake(scrollX, scrollH - 0.5f, scrollW, 0.5f);
}
...
#pragma mark - HAScrollNavBarDelegate
- (void)navibar:(HAScrollNavBar *)navBar didSelectAtIndex:(NSInteger)index {
    if([self.delegate respondsToSelector:@selector(cursor:selectedAtIndex:)]) {
        [self.delegate cursor:self selectedAtIndex:index];
    }
}
