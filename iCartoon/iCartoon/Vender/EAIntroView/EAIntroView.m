...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.frame.size.height - self.pageControlY, self.frame.size.width, 40)];
    [self.pageControl setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
...
- (void)setNextButton:(UIButton *)nextButton {
    ...
    [_nextButton addTarget:self action:@selector(nextIntroduction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)skipIntroduction {
    if ([(id)self.delegate respondsToSelector:@selector(introDidSkip)]) {
        [self.delegate introDidSkip];
    }
    
    [self hideWithFadeOutDuration:0.3];
}

- (void)nextIntroduction {
    if ([(id)self.delegate respondsToSelector:@selector(introDidFinish)]) {
        [self.delegate introDidFinish];
    }
