...
- (id)initWithFrame:(CGRect)frame animationDuration:(NSTimeInterval)animationDuration {
    ...
    if (self) {
        ...
        _pageControl.pageIndicatorTintColor = I_COLOR_WHITE;
        _pageControl.currentPageIndicatorTintColor = I_COLOR_YELLOW;
...
- (void)loadData {
    ...
    for (int i = 0; i < 3; i++) {
        if([_curViews count] >= 3) {
            ...
            CGRect rect = v.frame;
            rect.origin = CGPointMake(v.frame.size.width * i, 0);
            v.frame = rect;
...
- (void)getDisplayImagesWithCurpage:(int)page {
    ...
    if(_datasource && _datasource && [_datasource pageAtIndex:pre]) {
...
- (void)setViewContent:(UIView *)view atIndex:(NSInteger)index {
    if(index == _curPage) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            [v addGestureRecognizer:singleTap];
            CGRect rect = v.frame;
            rect.origin = CGPointMake(CGRectGetWidth(v.frame) * i , 0);
            v.frame = CGRectOffset(rect, v.frame.size.width * i, 0);
            [_scrollView addSubview:v];
        }
    }
    if(_curPage == 0) {
