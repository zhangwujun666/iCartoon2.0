...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        self.borderColor = RGB(212, 212, 212);
...
-(CGRect)getViewFrame
{
    ...
    for (NSString *title in self.titleArray) {
        CGFloat width = [title boundingRectWithSize:CGSizeMake(260, 100) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:TITLE_FONT} context:nil].size.width;
        frame.size.width = MAX(width, frame.size.width);
    }
    if ([self.titleArray count] == [self.imageArray count]) {
        frame.size.width = 10 + 25 + 10 + frame.size.width + 40+10;
    }else{
        frame.size.width = 10 + frame.size.width + 40;
    }
    
    frame.origin.x = self.showPoint.x - frame.size.width/2;
    frame.origin.y = self.showPoint.y + 2.0f;
    ...
    //右间隔最小5x
    CGFloat width = [UIScreen mainScreen].bounds.size.width - 1.0f;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.textColor = I_COLOR_GRAY;
